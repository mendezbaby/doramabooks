...
const EMAIL_KEY = "doramasbook_email";
...
function openModal() {
  ...
  if (getPurchase()) {
    ...
    // pre-preenche o e-mail se a pessoa já digitou antes
    $("#buyerEmail").value = localStorage.getItem(EMAIL_KEY) || "";
    showStep("resumo");
  }
}
...
async function createPixCharge() {
  // valida o e-mail (vira a "chave" da compra para recuperação)
  ...
  try {
    ...
    // este e-mail já pagou antes? libera na hora, sem cobrar de novo
    if (data.already_paid) {
      const txId = (data.transaction && (data.transaction.id || data.transaction.txid)) || "already-paid";
      onPaymentConfirmed(txId);
      return;
    }

    const tx = data.transaction;
    if (!tx) throw new Error("Resposta inesperada do servidor");
...
$("#linkRecuperar").addEventListener("click", () => {
  $("#recoverEmail").value = localStorage.getItem(EMAIL_KEY) || "";
  const msg = $("#recoverMsg");
  msg.classList.add("hidden");
  showStep("recuperar");
});

$("#linkVoltar").addEventListener("click", () => showStep("resumo"));
...
$("#btnRecuperar").addEventListener("click", async () => {
  ...
  try {
    ...
    if (resp.ok && data.success && data.paid) {
      // compra confirmada na NexusPag: libera o acesso neste navegador
      localStorage.setItem(EMAIL_KEY, email);
      savePurchase("recovered-" + email);
      showStep("sucesso");
      confettiBurst();
      toast("🎉 Acesso recuperado! Boa leitura 💜", 4000);
    } else {
      msg.textContent = "😕 Nenhuma compra confirmada para este e-mail. Confira se digitou certinho.";
      msg.className = "recover-msg error";
    }
  } catch (err) {
...
// permite confirmar com Enter nos campos de e-mail
$("#buyerEmail").addEventListener("keydown", (e) => {
  if (e.key === "Enter") createPixCharge();
});
